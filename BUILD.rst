...
The glad OpenGL loader is generated as part of the build; consequently,
``glad/gl.h`` is not present in a clean checkout. CMake retrieves glad source
code and runs its generator using the selected Python interpreter. The
interpreter requires Jinja2, but it does not require a separately installed
``glad2`` Python package for this CMake-based workflow.
...
Here is the recommended Windows workflow for this ``vs2026`` branch. Both the
Visual Studio workflow and the command-line workflow use Ninja; do not generate
a traditional Visual Studio solution for this branch.
...
3. Create the repository-local Python environment required for glad 2 code
   generation. Python 3.12 is the recommended interpreter for this environment:
   ...
   The final command must print a Jinja2 version number. The local
   ``.venv-glad`` directory is ignored by Git and must not be committed.
...
4. If you configured this checkout with an older dependency stack, Visual
   ...
   third-party source/build state, not assignment source code.

Building directly in Visual Studio 2026
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The repository includes ``CMakeSettings.json`` for opening the source folder in
Visual Studio. It configures:

* Ninja as the generator;
* ``out\build\x64-Debug`` as the Debug build tree;
* ``out\build\x64-Release`` as the RelWithDebInfo build tree;
* ``.venv-glad\Scripts\python.exe`` as the Python interpreter used by glad
  generation.

After completing steps 1--4:

1. In Visual Studio 2026, choose **Open a local folder** and select the
   ``CG_Labs`` repository root.
2. Wait for CMake configuration to complete. Warnings involving deprecated
   ``FetchContent_Populate`` calls do not by themselves mean that configuration
   failed; configuration has succeeded when CMake reports that build files were
   written to ``out\build\x64-Debug``.
3. Select an executable target, for example ``EDAF80_Assignment1.exe``.
4. Choose **Build > Build All** or press ``Ctrl+Shift+B``.
5. Run the selected assignment using ``F5`` or ``Ctrl+F5``.

The first Visual Studio build can take several minutes because it retrieves and
compiles the dependencies and creates generated files such as
``out\build\x64-Debug\gladsources\glad_gl_core_46\include\glad\gl.h``.

Building from Developer PowerShell with Ninja
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

To configure a separate Debug build tree outside Visual Studio's ``out``
directory, run:
...
.. code-block:: powershell
   ...
   cmake -S . -B build -G Ninja `
       ...
       -DPython3_EXECUTABLE="$PWD\.venv-glad\Scripts\python.exe" `
...
``CMAKE_BUILD_TYPE`` must be specified during configuration. The policy-version
option protects configuration against policy requirements in fetched CMake
projects.
...
Build all assignments, or build one intended target:
...
The Windows/Linux runtime path requests an OpenGL 4.6 core context. If context
creation fails, update the graphics driver or change the requested version in
``src/core/WindowManager.cpp`` for older hardware.
...
If Visual Studio displays many OpenGL errors beginning with ``cannot open source
file "glad/gl.h"``, inspect **Build Output** before attempting to change source
code. The missing-header errors are normally a consequence of a failed glad
generation step. In a clean checkout, ``glad/gl.h`` is generated only during
the first successful build.

If the failed glad generation command uses an unexpected Python interpreter,
or Python reports a missing Jinja2 dependency, verify the repository-local
environment:
...
For a Visual Studio build, also confirm that ``CMakeSettings.json`` points
``Python_EXECUTABLE`` and ``Python3_EXECUTABLE`` to
``${projectDir}\.venv-glad\Scripts\python.exe``. After correcting the
environment, remove the failed Visual Studio cache and build tree, then reopen
...
If the executable renders correctly but Dear ImGui checkboxes, sliders or
windows cannot be clicked or dragged, verify that the GLFW mouse callbacks in
``src/core/WindowManager.cpp`` forward both mouse-button and cursor-position
events to ``ImGui_ImplGlfw_*Callback``. The ``vs2026`` branch contains this
...
5. Run the intended assignment executable from its generated output location.
   ...
   terminal output and generated ``log.txt`` file.
