	...
	unsigned int const vertical_split_count)
{
	...
	float const d_horizontal = width / static_cast<float>(horizontal_split_edges_count);
	float const d_vertical = height / static_cast<float>(vertical_split_edges_count);

	// generate vertices iteratively
	size_t index = 0u;
	float horizontal_len = 0.0f;
	float vertical_len = 0.0f;
	for (unsigned int i = 0u; i < vertical_split_vertices_count; ++i) {

		for (unsigned int j = 0u; j < horizontal_split_vertices_count; ++j) {
			// vertex
			vertices[index] = glm::vec3(horizontal_len,
				0.0f,
				vertical_len);

			// texture coordinates
			texcoords[index] = glm::vec3(static_cast<float>(j) / (static_cast<float>(horizontal_split_vertices_count)),
				0.0f,
				static_cast<float>(i) / (static_cast<float>(vertical_split_vertices_count)));

			horizontal_len += d_horizontal;
			++index;
		}
		horizontal_len = 0.0f;
		vertical_len += d_vertical;
	}
                                ...
                                unsigned int const latitude_split_count)
{
	...
	// create index array
	auto index_sets = std::vector<glm::uvec3>(2u * latitude_slice_edges_count * longitude_slice_edges_count);
                               ...
                               unsigned int const minor_split_count)
{
	...
	auto const major_slice_edges_count = major_split_count + 1u;  // quads around ring
	auto const minor_slice_edges_count = minor_split_count + 1u;  // quads around tube
	auto const major_slice_vertices_count = major_slice_edges_count + 1u; // seam dupe
	auto const minor_slice_vertices_count = minor_slice_edges_count + 1u; // seam dupe
	...
	for (unsigned int i = 0u; i < major_slice_vertices_count; ++i) {
		...
		for (unsigned int j = 0u; j < minor_slice_vertices_count; ++j) {
			...
			// Texture coordinates: s along major (v), t along minor (u)
			texcoords[index] = glm::vec3(
				static_cast<float>(j) / static_cast<float>(minor_slice_vertices_count),
			...
			// Binormal (d/du): direction around the tube
			// d/du: (-r sin u sin v, -r cos u, -r sin u cos v)
			...
			// Normal from T¡ÁB (same order as sphere code)
			auto const n = glm::cross(tangents[index], binormals[index]);
			normals[index] = glm::normalize(n);
	...
	// Create index array: two triangles per quad cell
	auto index_sets = std::vector<glm::uvec3>(2u * minor_slice_edges_count * major_slice_edges_count);
	...
	// ---- Buffer/VAO setup: identical to createSphere ----
	bonobo::mesh_data data;
	glGenVertexArrays(1, &data.vao);
	assert(data.vao != 0u);
	glBindVertexArray(data.vao);
	...
	auto const binormals_size = static_cast<GLsizeiptr>(binormals.size() * sizeof(glm::vec3));
	auto const bo_size = static_cast<GLsizeiptr>(
		vertices_size + normals_size + texcoords_size + tangents_size + binormals_size
		);
